---
title: "Insomnia NMA"
subtitle: "Prepared by Virginia Chiocchia on 16th August, refers to a single outcome analysis (Total number of dropouts) in a frequentist setting"
output: 
  word_document: 
    fig_height: 7
    fig_width: 10
...
```{r, include=FALSE}
#prepare the data
DATApairs=pairwise(treat=Drug_name, event=as.numeric(DO_total), n=N_arm_total_stapf, data=DATA, studlab = Study_name, sm= "OR")
...
DPs <- DPs[-grep("LAT|MAT|BBT I|Mindfulness|massage|juice|control?",DPs$treat1),]
# remove studies with "special patients" that should not be included in main analysis
DPs <- DPs[DPs$Included==1,]



DOtot<-netmeta(TE,seTE,treat1,treat2,studlab,data=DPs,  sm="OR",r="pbo",comb.fixed =F, comb.random = T)
DOtot
...
Below is a description for the network formed by studies examining the outcome Total Number of Dropouts
...
The multiarm study Dement 1991 was also excluded because data was only available for one comparison.
\    
...
```{r, echo=FALSE}
#network plot
netgraph(DOtot, plastic=F, thickness="number.of.studies", multiarm = F, points=T, col=1, number.of.studies=T)
```
...
#forest plots
forest(DOtot, ref="pbo", sortvar = -Pscore,xlab="OR")
```

```{r, echo=FALSE, message=FALSE, warning=FALSE}

# create league table and export as excel
leagueDOtot <- netleague(DOtot, digits = 2, bracket = "(", separator = " to ")
write.xlsx(leagueDOtot$random, file = "leagueDOtot-random.xls",
...
```{r, echo=FALSE}
cat(paste("tau=",round(DOtot$tau,3)))
```
and I-square (total) is 
```{r, echo=FALSE}
cat(paste("I2=",round(DOtot$I2, digits = 2)*100,"%"))
```
