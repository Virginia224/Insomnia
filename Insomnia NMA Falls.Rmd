---
title: "Insomnia NMA"
subtitle: "Prepared by Virginia Chiocchia on 16th August, refers to a single outcome analysis (Falls) in a frequentist setting"
output: 
  word_document: 
    fig_height: 7
    fig_width: 10
...
```{r, include=FALSE}
#prepare the data
DATApairs=pairwise(treat=Drug_name, event=N_falls, n=`N_arm_total_for sideeffects`, data=DATA, studlab = Study_name, sm= "OR")
...
falls<-netmeta(TE,seTE,treat1,treat2,studlab,data=DPs,  sm="OR",r="pbo",comb.fixed =F, comb.random = T)
falls
...
Below is a description for the network formed by studies examining the outcome Falls.
...
The multiarm study Glass 2008 was also excluded because data was not available for all comparisons.
\    
...
```{r, echo=FALSE}
#network plot
netgraph(falls, plastic=F, thickness="number.of.studies", multiarm = F, points=T, col=1, number.of.studies=T)
```
...
#forest plots
forest(falls, ref="pbo", sortvar = -Pscore,xlab="OR")
```
```{r, echo=FALSE, message=FALSE, warning=FALSE}

# create league table and export as excel
leagueFalls <- netleague(falls, digits = 2, bracket = "(", separator = " to ")
write.xlsx(leagueFalls$random, file = "leagueFalls-random.xls",
